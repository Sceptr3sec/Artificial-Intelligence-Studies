...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def __init__(self, 
        ...
        # Attach an OpenAI embedding function at creation time. We still embed
        # each chunk manually via get_embedding() and pass embeddings explicitly
        # in collection.add() (per the assignment's explicit embedding-creation
        # requirement), so this function is never actually invoked for documents.
        # But Chroma PERSISTS which embedding function a collection was created
        # with, and refuses to attach a different one later — so this is required
        # to let rag_client.py query this same collection with matching OpenAI
        # query embeddings instead of Chroma's local default embedding space.
        ...
        while start < text_length:
            ...
            if end < text_length:
                ...
                for punct in [". ", "! ", "? ", "\n\n"]:
                    ...
                    if idx > best_break:
                        best_break = idx
                if best_break != -1 and best_break > self.chunk_size * 0.5:
                    end = start + best_break + 2  # include the punctuation

            chunk_str = text[start:end].strip()
            if chunk_str:
                chunk_metadata = dict(metadata)
                chunk_metadata["chunk_index"] = chunk_index
                chunks.append((chunk_str, chunk_metadata))
                chunk_index += 1

            if end >= text_length:
                break

            # Move start forward, backing up by the overlap amount
            start = max(end - self.chunk_overlap, start + 1)

        # Now that we know the total chunk count, record it on every chunk
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        # 'replace' mode: wipe out any existing documents from this file first
        ...
        for batch_start in range(0, len(documents), batch_size):
            ...
            batch_ids, batch_texts, batch_metadatas, batch_embeddings = [], [], [], []
            ...
            for text, metadata in batch:
                ...
                # New document (or replaced) - queue for batch add
